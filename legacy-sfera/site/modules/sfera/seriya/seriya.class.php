...
class seriya extends aModule
{
    function execute($arr)
    {
        // Получаем ID серии из GET параметра
        $seriya_id = isset($_GET['seriya_id']) ? (int)$_GET['seriya_id'] : 0;
        
        // Если не нашли в $_GET, пробуем получить из $arr
        if ($seriya_id == 0 && isset($arr['send_params']['seriya_id'])) {
            $seriya_id = (int)$arr['send_params']['seriya_id'];
        }
        ...
        // Получаем информацию о серии из view v_seriya по ID
        $seriya_id_safe = (int)$seriya_id;
        ...
        // Получаем название серии для поиска товаров
        $seriya_name_safe = noSQL($seriyaInfo['name']);
        
        // Получаем все товары этой серии по названию (так как в attributes хранится название)
        ...
        $seriyaData = [
            'id' => intval($seriyaInfo['id']),
            'name' => $seriyaInfo['name'],
            'count' => intval($seriyaInfo['count'])
        ];
        
        $_SESSION['smarty']->assign('seriya', $seriyaData);
        ...
        $_SESSION['smarty']->assign('pageTitle', 'Товары серии: ' . $seriyaInfo['name']);
        ...
        $templatePath = 'site/modules/sfera/seriya/seriya.tpl';
        if (!file_exists($templatePath)) {
            die("ERROR: Template not found: $templatePath");
        }
        
        try {
            $_SESSION['smarty']->display('sfera/seriya/seriya.tpl');
        } catch (Exception $e) {
